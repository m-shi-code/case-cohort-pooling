...
single.Rboot <- function(sim,ind){
  ...
  summary(glm2<- glm(status1 ~E+as.factor(age),data=sim.cch2, family=binomial))
...
pool.Rboot <- function(fam.id,ind,ccoh.pool){
  fam.id<- fam.id[ind]
  ...
  coeftest(glm3.frac1, vcov. = vcovCL(glm3.frac1, cluster = ccoh.pool2$pools, type = "HC0"))[2,]
}
...
###################################
  ...
  if (!bin.E){ E<- exp(rnorm(n.data*1.2))
  E <- E[E<= log(1/base.risk)/beta.e]
  E <- E[1:n.data]
  } else E <- sample(c(0,1),n.data,replace=T,prob=c(1-fr.e,fr.e))
  ...
  ##############################################
  #### 2.3: Logistic regression to analyze  ####
  ##############################################
   ...
   n.age.end <- table(ccoh.subc$age.start)  ## n.age.end here is actually n.age.start
    ...
    write.table(E.pool,col.names = F,sep='\t',quote=F,file='Exposure_in_pools.txt')
